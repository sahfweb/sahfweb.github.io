        ...
        <div class='event-all'>
            ...
            if (mysqli_num_rows($sql_cursor) == 0) {
                ...
                while ($row = mysqli_fetch_assoc($sql_cursor)) {
                    ...
                    $noticeDate = strtotime($row['noticeDate']); // Convert notice date to a Unix timestamp
                    $currentDate = time(); // Current Unix timestamp
                
                    // Calculate the difference in days between notice date and current date
                    $dateDifference = ($currentDate - $noticeDate) / (60 * 60 * 24);
